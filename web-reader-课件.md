...
**A. 基础（必做）**
1. 本地跑起服务：`cd services/web-reader && npm install && npm test`
2. 用 `POST /v1/parse` 解析 3 个真实网页（一个新闻页、一个 SPA、一个
   返回 403 的页面），记录每次响应里的 `engine` / `upgraded` / `extraction`
   字段，解释为什么

**B. 思考（选做）**
3. 阅读 `isRecoverableFailure`（engine.ts）：为什么 `unsafe_destination`
   被排除在"可恢复"之外？如果允许对它重试，攻击面会发生什么变化？
4. 设计题：如果要给 web-reader 加结果缓存（相同 URL 短期复用），
   缓存 key 应该包含什么？TTL 怎么定？会和 SSRF 防护的哪部分产生
   微妙的相互作用？

**C. 挑战（加分）**
5. 找一个 web-reader 提取失败的页面，分析失败在管线哪一步，提一个
   改进方案（改清洗规则/调 Readability/改升级阈值），并写测试证明
