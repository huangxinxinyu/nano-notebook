...
5. Inspect the Reasoning Trace and verify the Grounded Answer through Citations.
...
- A ready Source exposes a short generated overview and its original or normalized content for inspection.
...
- The interface shows high-level stages such as retrieving evidence, comparing Sources, verifying support, and writing the answer.
- An expandable Reasoning Trace shows retrieval queries, inspected Sources and passages, concise evidence-selection rationale, comparisons, uncertainty, and a conclusion summary.
- The Reasoning Trace is an audit-oriented product explanation, not a claim to expose hidden model cognition.
...
- Selecting a Citation opens the Source at the corresponding page, slide, section, transcript timestamp, image region, or passage with surrounding context.
...
- Provider-specific retention, backups, operator access, and deletion guarantees are technical and policy decisions for the next grilling session.
...
3. The user opens the normalized or original content and sees its overview.
4. The user creates a Chat, adjusts selected Sources, and asks a cross-Source question.
5. The Research Agent visibly progresses through research stages.
6. The user inspects the Reasoning Trace and receives a Grounded Answer.
...
6. Durable multi-step Research Agent runs and Reasoning Traces
