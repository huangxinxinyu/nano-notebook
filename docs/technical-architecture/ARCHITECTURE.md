...
The promoted configuration normally reranks every fused candidate set. A transient reranker failure receives bounded retry and may then return the unchanged RRF order under an explicit, traced degradation outcome; it never fabricates reranker scores or relaxes Citation and groundedness validation. A degraded search may support a grounded claim but can never establish the zero-support condition required for model-knowledge fallback.
...
If complete retrieval with no Retrieval Degradation and bounded refinement establishes that no selected Evidence supports the question at all, the Run may choose the `model_knowledge_fallback` Grounding Outcome. It performs a fresh Model Call using the source-less prompt and Chat context but no Source passage, accepted Evidence content, or Citation handle. The whole Answer opens with disclosure that it is not based on the selected Sources and contains no Citations. Partial support remains on the grounded path, while incomplete retrieval, deletion, authorization loss, and cancellation fail or disclose insufficiency rather than triggering fallback.

The source-less path has no Citations or Run Evidence Set. Its publication transaction still revalidates private Chat creator ownership and current Notebook membership, then inserts exactly one Assistant Message while completing the Run and Job. A fallback Run additionally revalidates every pinned Source and Evidence Revision so deletion or authorization change cannot manufacture eligibility. Sprint 3 removes the Message `answer_mode`; the producing Run's Grounding Outcome, pinned evidence, verified Claim Support Records, Publication Barrier, and Citations establish answer basis without duplicating a mode on the Message. Failed Runs publish no Assistant Message.
...
Each Chat belongs to one creator and remains private even from the Notebook Owner. In Sprint 2A, a submitted User Message creates a source-less model-knowledge Run. Runs with selected Sources pin a fixed Run Evidence Set and may publish either a verified Grounded Answer or the narrowly permitted zero-support Model-Knowledge Answer; Source-selection changes apply only to later Runs. Only the Publication Barrier can add the corresponding Assistant Message. Failed or stopped Runs can be retried as new Runs without editing history.
