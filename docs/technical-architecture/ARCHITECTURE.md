...
The source-less path has no Citations or Run Evidence Set. Its publication transaction still revalidates private Chat creator ownership and current Notebook membership, then inserts exactly one Assistant Message while completing the Run and Job. Sprint 3 removes the Message `answer_mode`; the workspace discloses that the current capability is not based on Notebook Sources. Failed Runs publish no Assistant Message. When grounded answering is implemented, the producing Run, pinned evidence, Publication Barrier, and validated Citations establish Grounded Answer truth without duplicating a mode on the Message.
