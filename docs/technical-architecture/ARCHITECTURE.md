...
The promoted configuration normally reranks every fused candidate set. A transient reranker failure receives bounded retry and may then return the unchanged RRF order under an explicit, traced degradation outcome; it never fabricates reranker scores or relaxes Citation and groundedness validation. A degraded search that returns citeable Evidence activates the strict grounded contract; one that returns no Evidence may leave the Run on the claim-free `source_free` path.
...
Before entering that transaction, grounded composition produces answer text, Citation handles drawn only from accepted Evidence Search Action results, and typed Claim Support Records for factual and synthesized claims. Deterministic validation resolves every handle within the Run Evidence Set and checks marker placement, continuous Unit ranges, and material multi-Source coverage. A separate Model Call with an independent prompt and structured schema verifies support and coverage from only the claim, cited Evidence, and required local context; it receives no Composer rationale or hidden reasoning. Composer and Verifier model, prompt, and schema versions are pinned independently in the Run configuration, but the first release may use the same base model for both roles. Failed verification returns the Agent to bounded research or revision; at budget exhaustion the claim is removed or replaced by an explicit insufficient-evidence statement. The Publication Barrier accepts only a final verified draft and still independently revalidates authoritative Citation identity and availability.

Before any accepted Evidence Search result contains a valid Evidence range, a selected-Source Run may publish a claim-free `source_free` Answer from the current Composer response. This branch carries no Citations and cannot claim Source support; it remains available after an empty, failed, or degraded no-Evidence search. Once any valid Evidence range is accepted, every subsequent final response uses provider-enforced grounded JSON and the existing claim-support and Citation barriers. Partial support remains on the grounded path, while deletion, authorization loss, and cancellation prevent publication.

The source-less path has no Citations or Run Evidence Set. A selected-Source `source_free` Run retains its pinned Evidence Set and revalidates every pinned Source and Evidence Revision even though it publishes no claim or Citation. Both publication paths revalidate private Chat ownership and current Notebook membership, then insert exactly one Assistant Message while completing the Run and Job. The producing Run's Grounding Outcome, pins, verified Claim Support Records, Publication Barrier, and Citations establish answer basis without duplicating a mode on the Message. Failed Runs publish no Assistant Message.
...
Citation interaction uses one Source Viewer shell with format-specific adapters. The same shell opens a ready Source directly from the Sources list for full inspection, including Evidence Coverage warnings, while Citation navigation additionally focuses the addressed Evidence Unit or range. PDF, rendered PPTX slides, and images resolve source-native regions; audio and YouTube resolve immutable transcript intervals; HTML, DOCX, TXT, and Markdown resolve normalized structural blocks or ranges. Hover previews use bounded authoritative Evidence excerpts. Missing coordinates fall back to the actual Evidence Unit without invented precision, and resolution never refreshes an external Source or grants original-file download.
...
Each Chat belongs to one creator and remains private even from the Notebook Owner. In Sprint 2A, a submitted User Message creates a source-less model-knowledge Run. Runs with selected Sources pin a fixed Run Evidence Set and may publish either a verified Grounded Answer after accepting citeable Evidence or a claim-free `source_free` Answer while no such Evidence exists; Source-selection changes apply only to later Runs. Only the Publication Barrier can add the corresponding Assistant Message. Failed or stopped Runs can be retried as new Runs without editing history.
